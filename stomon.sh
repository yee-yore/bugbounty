...
show_help_full() {
    cat << EOF
STOMON v3.0 - Subdomain Takeover Monitor
Author: yee-yore
=====================================
...
log() {
    ...
    case $level in
        ...
        INFO) 
            # Skip INFO logs in quiet mode unless they contain important keywords
            if [ "$QUIET_MODE" = "true" ]; then
                # Only show scanning progress, completion, and important status
                if [[ "$message" =~ "Scanning..." ]] || \
                   [[ "$message" =~ "Starting:" ]] || \
                   [[ "$message" =~ "Completed:" ]] || \
                   [[ "$message" =~ "All scans completed" ]] || \
                   [[ "$message" =~ "Scanning.*domains from" ]]; then
...
parallel_baddns_scan() {
    ...
    local workers="$3"
    local total_subs="$4"
    local show_progress="${5:-true}"  # Default to showing progress
    local progress_file="${6:-}"  # Optional progress file for list mode
    ...
    # Progress monitor (only if not in quiet mode or explicitly requested)
    local progress_pid=""
    if [ "$show_progress" = "true" ] && [ "$QUIET_MODE" != "true" ]; then
        (
            ...
            while read -r line < "$progress_pipe"; do
                ((completed++))
                [ $completed -gt $total_subs ] && completed=$total_subs
        ...
        # Consume progress messages without displaying (but write to file if specified)
        (
            local completed=0
            while read -r line < "$progress_pipe"; do
                ((completed++))
                ...
                if [ -n "$progress_file" ]; then
                    echo "scanning|$completed|$total_subs|$percent" > "$progress_file"
                fi
            done
        ) &
        progress_pid=$!
    ...
    echo "done" > "$progress_pipe" 2>/dev/null || true
    sleep 0.1  # Allow progress monitor to read final message
...
run_scan() {
    ...
    # Progress file for list mode
    local progress_file=""
    if [ -n "$PROGRESS_DIR" ] && [ -d "$PROGRESS_DIR" ]; then
        progress_file="$PROGRESS_DIR/${domain}.progress"
        echo "starting|0|0|0" > "$progress_file"
    fi
    ...
    if [ "$total_subs" -gt 0 ]; then
        ...
        if [ "$BADDNS_WORKERS" -gt 1 ]; then
            # Don't show individual progress in quiet mode (list mode)
            local show_progress="true"
            [ "$QUIET_MODE" = "true" ] && show_progress="false"
            parallel_baddns_scan "subdomains.txt" "baddns_raw.json" "$BADDNS_WORKERS" "$total_subs" "$show_progress" "$progress_file"
        else
            # Sequential scanning
            > baddns_raw.json
            local count=0
            while IFS= read -r subdomain; do
                ...
                # Update progress file if in list mode
                if [ -n "$progress_file" ]; then
                    echo "scanning|$count|$total_subs|$percent" > "$progress_file"
                else
    ...
    if [ -n "$progress_file" ]; then
        echo "complete|$confirmed_count|$((probable_count - confirmed_count))|$((possible_count - probable_count))" > "$progress_file"
    fi
            ...
            while [ ${#running_jobs[@]} -ge $CONCURRENCY ]; do
                ...
                for i in "${!running_jobs[@]}"; do
                    if kill -0 "${running_jobs[$i]}" 2>/dev/null; then
                        ...
                        ((completed++))
                        local done_domain="${running_domains[$i]}"
                        local progress_file="$PROGRESS_DIR/${done_domain}.progress"
                        
                        # Show completion with results
                        if [ -f "$progress_file" ]; then
                            local progress_data=$(cat "$progress_file" 2>/dev/null)
                            local status=$(echo "$progress_data" | cut -d'|' -f1)
                            if [ "$status" = "complete" ]; then
                                local confirmed=$(echo "$progress_data" | cut -d'|' -f2)
                                local probable=$(echo "$progress_data" | cut -d'|' -f3)
                                local possible=$(echo "$progress_data" | cut -d'|' -f4)
                                log INFO "[$completed/$total_domains] Completed: $done_domain - C:$confirmed P:$probable L:$possible"
                            else
                                log INFO "[$completed/$total_domains] Completed: $done_domain"
                            fi
                            rm -f "$progress_file"
                        else
                            log INFO "[$completed/$total_domains] Completed: $done_domain"
